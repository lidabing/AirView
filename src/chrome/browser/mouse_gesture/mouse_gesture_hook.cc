...
bool MouseGestureHook::Install(){
	...
	if (process_type.empty()){ //browser主进程
		Real_DispatchMessageW = (LRESULT(WINAPI*)(const MSG*))DetourFindFunction(user32, "DispatchMessageW");
		...
		RealPlugin_DispatchMessageW = (LRESULT(WINAPI*)(const MSG*))DetourFindFunction(user32, "DispatchMessageW");
		if (RealPlugin_DispatchMessageW)
			DetourAttach(&(PVOID&)RealPlugin_DispatchMessageW, MyPlugin_DispatchMessageW);
	}
	return true;
}
...
void MouseGestureHook::UnInstall(){
	...
	if (process_type.empty()){ //browser主进程
		...
		if (RealPlugin_DispatchMessageW)
			DetourDetach(&(PVOID&)RealPlugin_DispatchMessageW, MyPlugin_DispatchMessageW);
	}
}
