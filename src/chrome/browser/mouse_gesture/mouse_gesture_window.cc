...
void MouseGestureWindow::OnPaintSkia(){
	...
	::UpdateLayeredWindow(m_hWnd, NULL, lp_rect,
		reinterpret_cast<LPSIZE>(lp_rect + 1), memdc, &src_point, RGB(125, 125, 125), &blend, ULW_ALPHA);
...
LRESULT	MouseGestureWindow::OnPaint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
...
bool	MouseGestureWindow::OnRButtonDown(const MSG*	msg) {
	...
	if (pt.y > event_window_rect_.bottom || pt.x > event_window_rect_.right || pt.x < 0 || pt.y < 0)
		return true;
...
LRESULT MouseGestureWindow::OnDestroy(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	return 1;
}

LRESULT MouseGestureWindow::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	return 0;
}

int	MouseGestureWindow::Get2PointDistance(POINT& start_point, POINT& end_point){
	return static_cast<int>(sqrtl(abs(end_point.x - start_point.x)*abs(end_point.x - start_point.x)
