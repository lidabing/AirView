...
void TabStripModel::ExecuteContextMenuCommand(
  ...
  switch (command_id) {
	...
	case CommandCloseTabsToLeft: {
		content::RecordAction(
			UserMetricsAction("TabContextMenu_CloseTabsToLeft"));
		InternalCloseTabs(GetIndicesClosedByCommand(context_index, command_id),
			CLOSE_CREATE_HISTORICAL_TAB);
		break;
	}
...
std::vector<int> TabStripModel::GetIndicesClosedByCommand(
  ...
  DCHECK(ContainsIndex(index));
  DCHECK(id == CommandCloseTabsToRight || id == CommandCloseOtherTabs || id == CommandCloseTabsToLeft);
