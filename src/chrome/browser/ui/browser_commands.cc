...
void CloseTab(Browser* browser) {
  ...
  ///airview patch{
  if (1 == browser->tab_strip_model()->count()){
	  //创建一个新页面
	  //关闭最后一个TAB如果不是newtab页面那么不是关闭窗口
	  WebContents* content = browser->tab_strip_model()->GetWebContentsAt(0);
	  bool need_create_new_tab = !(content->GetURL().is_valid() &&
		  content->GetURL().SchemeIs(content::kChromeUIScheme) &&
		  content->GetURL().DomainIs("newtab"));
	  if (need_create_new_tab)
		  AddTabAt(browser, GURL(), -1, true);
